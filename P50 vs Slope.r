...
points(subset(dataS2, k==0.05 & MAP==1825 & d<=10, select=c("P50", "slope")), type="l", col=Cols[4])
points(subset(dataS3, k==0.05 & MAP==1825 & pkx==0.5 & h3==25 & d<=10, select=c("P50", "slope")), type="l", col=Cols[5])
points(subset(dataS3, k==0.05 & MAP==1825 & pkx==0.5 & h3==100 & d<=10, select=c("P50", "slope")), type="l", lty=2, col=Cols[5])
points(subset(dataS3, pkx==0.25 & d<=10, select=c("P50", "slope")), type="l", lwd=1, col=Cols[5])
points(subset(dataS3, pkx==0.75 & d<=10, select=c("P50", "slope")), type="l", lwd=4, col=Cols[5])
...
legend("topleft", legend=c("I", "III"), title="Scenario", lty=c(1), col=c(Cols[3], Cols[5]))
legend("top", legend=c("25", "100"), title=expression(beta), lty=c(1, 2), col="black", bg="white")
legend("topright", legend=c("25%", "50%", "75%"), title=expression(italic(p[k[x]])), lty=1, lwd=c(1, 2, 4), col=Cols[5])
