...
points(subset(dataS2, pkx==0.5 & h3==25, select=c("ps", "g1")), type="l", col=Cols[4])
points(subset(dataS2, pkx==0.5 & h3==100, select=c("ps", "g1")), type="l", lty=2, col=Cols[4])
points(subset(dataS2, pkx==0.25, select=c("ps", "g1")), type="l", lwd=1, col=Cols[4])
points(subset(dataS2, pkx==0.75, select=c("ps", "g1")), type="l", lwd=4, col=Cols[4])
points(dataS3, type="l", col=Cols[5])

axis(1, xlim=c(-6, 0), pos=-8*0.04, lwd=2)
mtext(expression(psi[s]~(MPa)),side=1,line=2.4, cex=1.3)
axis(2, ylim=c(0, 8), pos=-6, lwd=2, at=c(0, 2, 4, 6, 8))
mtext(expression(italic(g[1])~(kPa^-0.5)),side=2,line=1.8, cex=1.3)
legend("bottomright", c("Zhou et al. 2014"), lwd=1, lty=3, col="black")
